...
export async function captureImageSection(
  ...
  const auroraCanvas = imageSectionElement.querySelector('canvas') as HTMLCanvasElement;
  if (!auroraCanvas) {
  ...
  try {
    ...
    ctx.drawImage(
      auroraCanvas,
      0, 0, auroraCanvas.width, auroraCanvas.height,
      0, 0, width, height
    );
  } catch (error) {
    console.warn('Could not capture Aurora canvas directly, using fallback', error);
...
async function drawDOMContent(
  ...
  if (imgElement && imgElement.complete) {
    ...
    try {
      await drawImageFromURL(ctx, imgElement.src, x, y, containerRect.width, containerRect.height);
    } catch (error) {
  ...
  // Draw social handles
  const socialContainer = contentOverlay.querySelector('.flex.flex-row.gap-6') as HTMLElement;
  if (socialContainer) {
...
function drawText(
  ...
  wrapText(ctx, text, x, y, maxWidth, parseFloat(computedStyle.lineHeight) || 1.2);
 ...
 */
export function downloadImage(dataURL: string, filename: string = 'captured-image'): void {
